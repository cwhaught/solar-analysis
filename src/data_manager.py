...
class SolarDataManager:
    ...
    def __init__(self, csv_path: str, enphase_client, cache_dir: str = "data/processed"):
        """
    ...
    def load_csv_data(self, force_reload: bool = False) -> pd.DataFrame:
        ...
        self.logger.info(f"Loaded {len(df_kwh)} CSV records from {df_kwh.index.min()} to {df_kwh.index.max()}")

        return self._csv_data

    def load_api_data(self, days_back: int = 30, force_reload: bool = False) -> pd.DataFrame:
        """
        ...
        try:
            ...
            if not api_data.empty:
                ...
                api_data = api_data.rename(columns={'daily_energy_kwh': 'Production (kWh)'})
    ...
    def get_daily_production(self, start_date: Optional[datetime] = None,
                             end_date: Optional[datetime] = None,
                             source_priority: str = 'csv_first') -> pd.DataFrame:
        """
    ...
    def get_detailed_intervals(self, target_date: datetime,
            ...
            try:
                ...
                if not api_intervals.empty:
                    ...
                    standardized = pd.DataFrame({
                        'Production (kWh)': api_intervals.groupby(api_intervals.index)['energy_delivered_kwh'].sum()
                    })
                    standardized.attrs['source'] = 'api'
                    return standardized

            except Exception as e:
                self.logger.warning(f"Could not get API intervals for {target_date}: {e}")
    ...
    def update_from_api(self, save_cache: bool = True) -> Dict[str, int]:
        ...
        if not current_daily.empty:
            ...
            days_to_update = (datetime.now().date() - last_date.date()).days + 7  # Extra buffer
        else:
            days_to_update = 30
    ...
    def get_data_summary(self) -> Dict:
        ...
        summary = {
            'csv': {
                ...
                ) if not csv_data.empty else None,
                'granularity': csv_data.attrs.get('granularity', 'unknown') if not csv_data.empty else None
            },
            'api': {
                ...
                ) if not api_data.empty else None,
                'granularity': api_data.attrs.get('granularity', 'unknown') if not api_data.empty else None
            }
        }

        return summary

    def export_combined_dataset(self, filename: str,
                                source_priority: str = 'csv_first') -> bool:
        """
